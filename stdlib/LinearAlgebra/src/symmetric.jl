...
const ClosedUnderDivision = Union{AbstractFloat, Complex{<:AbstractFloat},
...
!!! compat "Julia 1.3"
    This function requires Julia 1.3 or later.
"""
symmetrize(X::AbstractMatrix{<:ClosedUnderDivision}; conjugate::Bool=true) =
    symmetrize!(copy(X), conjugate=conjugate)
symmetrize(X::AbstractMatrix; conjugate::Bool=true) =
    symmetrize!(float(X), conjugate=conjugate)
